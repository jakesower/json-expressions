...
describe("$get", () => {
  describe("evaluate form", () => {
    ...
    it("returns default when path does not exist", () => {
      expect(
        evaluate({
          $get: { object: { age: 30 }, path: "name", default: "defaultName" },
        }),
      ).toEqual("defaultName");
    });
  ...
  describe("apply form", () => {
    ...
    it("gets value with default when path exists using object form", () => {
      expect(
        apply(
          { $get: { path: "name", default: "defaultName" } },
          { name: "Kenji", age: 25 },
        ),
      ).toEqual("Kenji");
    });
    ...
    it("returns default when path does not exist using object form", () => {
      expect(
        apply({ $get: { path: "name", default: "defaultName" } }, { age: 25 }),
      ).toEqual("defaultName");
    });
    ...
    it("returns default when nested path does not exist using object form", () => {
      expect(
        apply(
          { $get: { path: "user.name", default: "Anonymous" } },
          { user: {} },
        ),
      ).toEqual("Anonymous");
    });
    ...
    it("works with complex default values using object form", () => {
      expect(
        apply(
          {
            $get: { path: "user", default: { name: "Guest", role: "visitor" } },
          },
          { age: 25 },
        ),
      ).toEqual({ name: "Guest", role: "visitor" });
    ...
    it("handles undefined vs null correctly using object form", () => {
      expect(
        apply({ $get: { path: "name", default: "default" } }, { name: null }),
      ).toEqual(null);
      expect(apply({ $get: { path: "name", default: "default" } }, {})).toEqual(
        "default",
      );
    });

    it("works without default in object form", () => {
      expect(apply({ $get: { path: "name" } }, { name: "Chen" })).toEqual(
        "Chen",
      );
      expect(
        apply({ $get: { path: "missing" } }, { name: "Chen" }),
      ).toBeUndefined();
    });
