  ...
  describe("$pick", () => {
    describe("apply form", () => {
      ...
      it("ignores missing properties", () => {
        expect(
          apply({ $pick: ["name", "missing", "age"] }, children[0]),
        ).toEqual({
          name: "Chen",
          age: 5,
        });
    ...
    describe("evaluate form", () => {
      ...
      it("throws error for invalid operand format", () => {
        expect(() => evaluate({ $pick: "not an array" })).toThrow(
          "$pick evaluate form requires array operand: [object, propertyNames]",
        );
      });
    });
  });
    ...
    describe("evaluate form", () => {
      ...
      it("throws error for invalid operand format", () => {
        expect(() => evaluate({ $omit: "not an array" })).toThrow(
          "$omit evaluate form requires array operand: [object, propertyNames]",
        );
      });
    });
  });
