...
describe("Custom Expressions", () => {
	...
	describe("mergeDefaults parameter", () => {
		it("creates engine with only custom expressions when mergeDefaults=false", () => {
			...
			expect(() => customOnlyEngine.apply({ $get: "name" }, { name: "test" }))
				.toThrowError(/Unknown expression operator: "\$get"/);
			
			expect(() => customOnlyEngine.apply({ $add: 5 }, 10))
				.toThrowError(/Unknown expression operator: "\$add"/);
